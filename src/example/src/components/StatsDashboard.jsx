...
const StatsDashboard = () => {
  ...
  useEffect(() => {
    console.log('🎯 DEBUG: StatsDashboard useEffect triggered - fetching stats...');
    const fetchStats = async () => {
      try {
        console.log('🔄 DEBUG: Starting to fetch stats from API...');
        setLoading(true);
        ...
        console.log('🔍 DEBUG: Extracted values:', { messagesProcessed, timeSaved });
        
        setStats({
          messagesProcessed,
          timeSaved,
        });
        console.log('📊 DEBUG: Stats updated in state:', { messagesProcessed, timeSaved });
        setLoading(false);
      } catch (error) {
  ...
  const AnimatedCounter = ({ value, duration = 2000 }) => {
    ...
    useEffect(() => {
      console.log('🎬 DEBUG: AnimatedCounter useEffect triggered with value:', value);
      let startTime;
      const animateCount = (timestamp) => {
        if (!startTime) startTime = timestamp;
        const progress = Math.min((timestamp - startTime) / duration, 1);
        const newCount = Math.floor(progress * value);
        console.log('🎬 DEBUG: Animation progress:', { progress, newCount, value });
        setCount(newCount);
        if (progress < 1) {
  ...
  const StatCard = ({ icon, title, value, unit, color }) => {
    console.log('📋 DEBUG: StatCard rendered with:', { title, value, loading });
    return (
      <motion.div
